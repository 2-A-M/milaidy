...
export interface PiAiModelOption {
  ...
  id: string;
  /** Human-readable model name from pi-ai's model catalog. */
  name: string;
  /** Provider id (e.g. anthropic, openai, openai-codex). */
  provider: string;
  /** Whether this entry matches ~/.pi/agent/settings.json default model. */
  isDefault: boolean;
}
...
export async function listPiAiModelOptions(): Promise<{
