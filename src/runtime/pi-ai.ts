...
export async function registerPiAiRuntime(
  ...
  // If credentials are missing, fall back to the pi settings default so the
  // user's pi OAuth/API-key based providers are used instead.
  const validatedModelSpec =
    opts.modelSpec &&
    piCreds.hasCredentials(parseModelSpec(opts.modelSpec).provider)
      ? opts.modelSpec
      : undefined;

  const largeSpec = opts.largeModelSpec ?? validatedModelSpec ?? defaultSpec;
  const smallSpec = opts.smallModelSpec ?? validatedModelSpec ?? largeSpec;
